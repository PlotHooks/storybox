    ...
    <body class="min-h-screen bg-[#050505] text-[#e9dcc2] antialiased">
                ...
                <div class="grid gap-5 xl:grid-cols-[22rem_minmax(0,1fr)]">
                    <aside class="overflow-hidden border-y border-[#3a2f1e] bg-[#0b0b0c]/95 shadow-[0_30px_80px_rgba(0,0,0,0.45)] sm:rounded-[2rem] sm:border">
                        <div class="border-b border-[#3a2f1e] px-5 py-5">
                            <div class="text-xs uppercase tracking-[0.35em] text-[#c8ac75]">DM History</div>
                            <h1 class="mt-2 text-2xl font-semibold text-[#fff2cc]">Transcript Navigator</h1>
                            <p class="mt-3 text-sm leading-6 text-[#cdbb98]">
                                Browse up to 30 days of retained direct messages. Active days show message counts and open read-only transcripts.
                            </p>
                        </div>
                            ...
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                                <div>
                                    ...
                                    <p class="mt-3 text-sm leading-6 text-[#cdbb98]">
                                        ...
                                        @else
                                            No direct messages were recorded for this day within the current 30-day history window.
                                        @endif
                                    </p>
                                </div>
        ...
        <script>
            ...
            function formatTsv(rows) {
                const headers = ['timestamp', 'sender_character_name', 'sender_character_id', 'dm_thread_id', 'message_type', 'body', 'roll_expression', 'roll_result', 'edited', 'deleted'];
            ...
            document.querySelectorAll('[data-history-action]').forEach((button) => {
                button.addEventListener('click', async () => {
                    ...
                    if (action === 'download-csv') {
                        downloadCsv(formatCsv(rows), `dm-{{ $room->slug }}-history-{{ $selectedDayString }}.csv`);
                    }
                });
            });
        </script>
