...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-[#050505] text-[#e9dcc2] antialiased">
        @php
            ...
            $profileRules = trim((string) ($room->profile_rules ?? ''));
        ...
        <div class="min-h-screen bg-[radial-gradient(circle_at_top,rgba(62,42,18,0.55),rgba(7,7,7,0.96)_45%,#050505_100%)]">
            <div class="w-full px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
                ...
                <section class="w-full overflow-hidden border-y border-[#3a2f1e] bg-[#0b0b0c]/95 shadow-[0_30px_80px_rgba(0,0,0,0.45)] sm:rounded-[2rem] sm:border sm:shadow-[0_30px_80px_rgba(0,0,0,0.45)]">
                            ...
                            <div class="max-w-4xl">
                                ...
                                @else
                                    <p class="mt-5 max-w-3xl text-base leading-7 text-[#d2c4a6] sm:text-lg sm:leading-8">
                                        This room profile has not been configured yet. Check back later, or add the first summary, joining information, and rules from the profile editor if you manage this room.
                                    </p>
                                @endif
                            </div>
                        </div>
                    ...
                    @if ($hasRoomProfile)
                        <div class="grid gap-8 border-t border-[#3a2f1e] px-6 py-8 sm:px-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:px-12 lg:py-10 xl:px-16">
                            ...
                            @if ($profileRules !== '')
                                <section>
                                    ...
                                    <div class="mt-4 whitespace-pre-line text-[15px] leading-7 text-[#d7c8aa] sm:text-base sm:leading-8">
                                        {{ $profileRules }}
                                    </div>
                                </section>
                            @endif
