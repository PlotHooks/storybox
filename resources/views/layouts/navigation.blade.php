<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Rooms dropdown (RPC-style switcher) -->
            <div
                ...
                x-data="{
                    ...
                    async toggleRooms() {
                        ...
                        if (this.openRooms && this.rooms.length === 0) {
                            this.loading = true;
                            try {
                                const res = await fetch('{{ route('rooms.sidebar') }}');
            ...
            >
                <div class="relative">
                    ...
                    <div
                        x-show="openRooms"
                        @click.outside="openRooms = false"
                        x-transition
                        class="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-md shadow-lg z-50"
                        style="display: none;"
                    >
                        <div class="py-1 text-sm max-h-72 overflow-y-auto">
                            <div class="px-3 py-1 text-[11px] text-gray-500 flex items-center justify-between border-b border-gray-100">
                                <span>#</span>
                                <span class="flex-1 text-center">Room name</span>
                            </div>
                            ...
                            <template x-for="room in rooms" :key="room.id">
                                ...
                                >
                                    <span class="w-6 text-[11px] text-gray-400" x-text="room.active_users || ''"></span>
                                    <span class="flex-1 truncate text-gray-800" x-text="room.name"></span>
                                </button>
                            </template>
                        </div>
                    </div>
