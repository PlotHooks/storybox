...
(function () {
    ...
    function archivedPreviewEntryHtml(entry) {
        const body = entry.body || entry.published?.body || entry.pending?.body || '';
        const tags = Array.isArray(entry.tags) && entry.tags.length > 0
            ? entry.tags
            : (Array.isArray(entry.pending?.tags) ? entry.pending.tags : []);

        return `
            <div class="rounded border border-[#332817] bg-[#101012] p-3">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <div class="mt-1 text-[10px] uppercase tracking-[0.14em] text-[#8f8675]">${escapeHtml(entry.category_label || 'Custom')} • ${escapeHtml((entry.status || 'draft').replace('_', ' '))}</div>
                    </div>
                    <span class="shrink-0 rounded border border-[#332817] bg-[#0b0b0c] px-2 py-0.5 text-[9px] uppercase tracking-[0.14em] text-[#8f8675]">${escapeHtml((entry.status || 'draft').replace('_', ' '))}</span>
                </div>
                ${tagsHtml(tags)}
                ${body ? `<div class="mt-3 whitespace-pre-wrap leading-relaxed text-[#d6c8ad]">${escapeHtml(body)}</div>` : '<div class="mt-3 text-sm text-[#8f8675]">No supplemental notes saved.</div>'}
            </div>
        `;
    }
    ...
    function renderEntryView(entry) {
        if (!entry) {
            ...
            subtitleEl.textContent = state.entries.length === 0
                ...
                : 'Select an entry or submit new room lore.';
            bodyEl.innerHTML = state.entries.length === 0
                ? '<div class="rounded border border-dashed border-[#332817] bg-[#101012]/60 p-4 text-[#8f8675]">No World Book entries yet.</div>'
                : '<div class="rounded border border-dashed border-[#332817] bg-[#101012]/60 p-4 text-[#8f8675]">No entry selected.</div>';
            renderActions(null);
            return;
        }
